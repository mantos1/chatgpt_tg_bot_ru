...
Чтобы получить ответ от бота в чате – тегните его: {bot_username} с запросом или в <b>ответе</b> на своё сообщение.
...
async def retry_handle(update: Update, context: CallbackContext):
    ...
    if len(dialog_messages) == 0:
        await update.message.reply_text("Нет сообщения для повторного запроса 🤷‍♂️")
        return
    ...
    async def message_handle_fn():
        ...
        try:
            ...
            if _message is None or len(_message) == 0:
                 await update.message.reply_text("🥲 Вы отправили <b>пустое сообщение </b>. Пожалуйста, повторите попытку", parse_mode=ParseMode.HTML)
                 return
...
def get_chat_mode_menu(page_index: int):
    ...
    text = f"Выберите <b>роль чат бота</b> ({len(config.chat_modes)} доступных ролей):"
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    for model_key in sorted(n_used_tokens_dict.keys()):
        ...
        details_text += f"- {model_key}: <b>{n_input_spent_dollars + n_output_spent_dollars:.03f}$</b> / <b>{n_input_tokens + n_output_tokens} токена</b>\n"

    # image generation
    image_generation_n_spent_dollars = config.models["info"]["dalle-2"]["price_per_1_image"] * n_generated_images
    if n_generated_images != 0:
        details_text += f"- DALL·E-2 (генерация изображений): <b>{image_generation_n_spent_dollars:.03f}$</b> / <b>{n_generated_images} - сгенерированных изображения</b>\n"
    ...
    if n_transcribed_seconds != 0:
        details_text += f"- Шепотом (распознавание голоса): <b>{voice_recognition_n_spent_dollars:.03f}$</b> / <b>{n_transcribed_seconds:.01f} секунд</b>\n"
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/retry", "Повторить последний запрос"),
